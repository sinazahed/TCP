...
User=root
WorkingDirectory=/
ExecStart=/RTT --iran  --lport:$IRPORT  --sni:$SNI --password:$TOPASS
Restart=always
...
User=root
WorkingDirectory=/
ExecStart=/RTT --kharej --iran-ip:$EXIP --iran-port:$IRPORTTT --toip:$TOIP --toport:$TOPORT --password:$TOPASS --sni:$SNI --terminate:$TERM
Restart=always
